...
function ready(datapoints) {
  ...
  // Add labels of each city (nested group)
  svg
    .selectAll('text')
    .data(nested)
    .enter()
    ...
    .attr('y', function(d) {
      ...
      // Finds the datapoint for month 12
      var decData = datapoints.find(d => +d.month === 12)
      console.log('december is', decData)

      // Use the yPositionScale and the high temp of Dec to position our text
      return yPositionScale(decData.high)
    })
    .attr('font-size', 12)
    .attr('dx', 5)
    // Mannually adjust positioning of text
    .attr('dy', function(d) {
      if ((d.key === 'Melbourne') | (d.key === 'Stockholm')) return 5
      else {
        return 0
      }
    })
    // Vertically centre-align with the dots
